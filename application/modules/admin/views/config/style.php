...
if(is_admin() || is_root())
{
	if(empty($_GET))
	{
		...
		if(!empty($template_names))
		{
			...
			echo '<h4>Select Template to Add style</h4>';
			echo '</div>';
			...
			{
				?>
				<a href="<?php echo base_url('admin/config/style').'?n='.$value.'&k='.encrypt($value); ?>"><h2><li><?php echo $value; ?></li></h2></a>
				<?php
			}
			echo '</ol>';
			echo '</div>';
		...
		if(decrypt($n, $k))
		{
			...
			$form->setParamName($n.'_style');
			$form->addInput('style','textarea');
			$form->setValue('style', '<style>/*type your style here</style>');
